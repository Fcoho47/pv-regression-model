"""
Train a RandomForestRegressor with holdout por plantas y guardar artefactos compatibles con el backend.

Usage:
  python train_model_rf.py --data-path datos_solcast_full-1764069958506.csv --run-name rf_run
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train RandomForest for regression with plant holdout.")
...
def main():
    ...
    print(f"Holdout plantas: {holdout_plants}")
    print(f"Filas holdout: {len(df_holdout_raw):,} ({holdout_frac:.1%})")
    ...
    run_name = args.run_name or f"rf_{args.n_estimators}_seed{args.seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
