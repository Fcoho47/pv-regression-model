"""
Train a LightGBM regressor with plant holdout and save artifacts compatible with the backend.

Usage:
  python train_model_lgbm.py --data-path datos_solcast_full-1764069958506.csv --run-name lgbm_run
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train LightGBM for regression with plant holdout.")
    ...
    p.add_argument("--learning-rate", type=float, default=0.07)
...
def main():
    ...
    run_name = args.run_name or f"lgbm_{args.n_estimators}_seed{args.seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
